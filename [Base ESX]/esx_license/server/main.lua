...
RegisterCommand("addweaponlicense", function(source, args, cb)
	...
	if xPlayer.job.name == 'police' then
		if xTarget ~= nil then
			...
			TriggerClientEvent('chat:addMessage', xPlayer.source, {template = '<div style="padding: 0.5vw; margin: 0.5vw; background-color: rgba(55, 69, 95, 0.5); border-radius: 3px;">[^4LABRP^0] You have given a weapons license to ^4ID:' .. targetId, '  </div>',});
		else
			TriggerClientEvent('chat:addMessage', xPlayer.source, {template = '<div style="padding: 0.5vw; margin: 0.5vw; background-color: rgba(55, 69, 95, 0.5); border-radius: 3px;">[^4LABRP^0] That ID is not online  </div>',});
		end
	else
		TriggerClientEvent('chat:addMessage', xPlayer.source, {template = '<div style="padding: 0.5vw; margin: 0.5vw; background-color: rgba(55, 69, 95, 0.5); border-radius: 3px;">[^4LABRP^0] You are not a police officer  </div>',});
	end
end)