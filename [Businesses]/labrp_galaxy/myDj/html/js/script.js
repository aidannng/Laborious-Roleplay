$(document).ready(function() {
    ...
    btn.click(function() {
        $.post('http://labrp_galaxy/togglePlaystate', JSON.stringify({}));
    ...
    directPlay.click(function() {
        $.post('http://labrp_galaxy/playNewSong', JSON.stringify({
            link: $('#linkInput').val()
        }));
        return;
    });
    ...
    buttonTwo.click(function() {
        $.post('http://labrp_galaxy/rewind', JSON.stringify({}));
        $('#timeLineInside').width($('#timeLineInside').width()-(perc*10))
        return;
    });

    buttonThree.click(function() {
        $.post('http://labrp_galaxy/forward', JSON.stringify({}));
        $('#timeLineInside').width($('#timeLineInside').width()+(perc*10))
        return;
    });

    buttonFour.click(function() {
        $.post('http://labrp_galaxy/down', JSON.stringify({}));
        return;
    });

    buttonFive.click(function() {
        $.post('http://labrp_galaxy/up', JSON.stringify({}));
        return;
    });
    ...
    AddPlaylist.click(function() {
        ...
        if ($('#CreatePlaylistName').val() != '') {
            ...
            $.post('http://labrp_galaxy/addPlayList', JSON.stringify({
    ...
    $(document).on('click', '.deleteSongs', function() {
        ...
        $.post('http://labrp_galaxy/deleteSong', JSON.stringify({
    ...
    $(document).on('click', '.deletePlaylist', function() {
        ...
        $.post('http://labrp_galaxy/deletePlaylist', JSON.stringify({
        ...
        if ($('#linkInput').val()) {
            ...
            $.getJSON('https://noembed.com/embed?url=', {format: 'json', url: $('#linkInput').val()}, function (data) {
                if (data.title) {
                    ...
                    $.post('http://labrp_galaxy/addSongToPlaylist', JSON.stringify({
                        id: i.data('id'),
                        link: $('#linkInput').val()
                    }));
                    ...
                    $.post('http://labrp_galaxy/noSongtitle', JSON.stringify({}));
                }
            });
    ...
    $(document).on('click', '.song', function() {
        ...
        $.post('http://labrp_galaxy/playSongFromPlaylist', JSON.stringify({
            id: $(this).data('id'),
            link: $(this).data('link'),
    ...
    function Close(event) {
        if (event.keyCode === 27) {
            $('body').hide()
            $.post('http://labrp_galaxy/close', JSON.stringify({}));
        }
    }
