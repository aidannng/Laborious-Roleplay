            ...
            if dst < 5 then
                ...
                if dst < 1.5 and not cutting then
                    ...
                    if IsControlJustPressed(0, 38) and not cutting then
                        cutting = true
                        TriggerEvent("kaves_drugs:cuttingMoney", Config.Laundry.cuttingZone.coords, Config.Laundry.cuttingZone.heading)
                    end
                end
            end
        Citizen.Wait(sleep)
...
RegisterNetEvent("kaves_drugs:cuttingMoney")
AddEventHandler("kaves_drugs:cuttingMoney", function(coord, heading)
    local playerPed = PlayerPedId()
    ...
    TriggerServerEvent("kaves_drugs:giveItem", "sorted_money", math.random(Config.Laundry.cuttingZone.countmin, Config.Laundry.cuttingZone.countmax))
    cutting = false
end)
                ...
                if IsControlJustPressed(0, 38) then
                    ESX.TriggerServerCallback("kaves_drugs:checkItemAll", function(output, count)
                        if output then
                            packaged = true
                            blackcount = count
                            TriggerServerEvent("kaves_drugs:removeItemAll", 'sorted_money')
                            TriggerEvent("kaves_drugs:packageMoney")
                        else
                            exports['mythic_notify']:SendAlert('error', 'You do not have a sorted money')
                        end
                    end, 'sorted_money')
                end
            end
...
RegisterNetEvent("kaves_drugs:packageMoney")
AddEventHandler("kaves_drugs:packageMoney", function()
    local playerPed = PlayerPedId()
    ...
    TriggerServerEvent("kaves_drugs:giveBlackMoney2", blackcount)
    blackcount = 0
    packaged = false
end)
    ...
    while true do
        ...
        if dst < 5 and not washing then
            ...
            if dst < 2 then
                ...
                if IsControlJustPressed(0, 38) then
                    ESX.TriggerServerCallback("kaves_drugs:checkBlackMoney", function(output)
                        if output then
                            washing = true
                            washcount = output
                            TriggerServerEvent("kaves_drugs:removeBlackMoney")
                            TriggerEvent("kaves_drugs:washMoney", Config.Laundry.washingZone.coord, Config.Laundry.washingZone.heading)
                        else
                            exports['mythic_notify']:SendAlert('error', 'You do not have a black money')
...
RegisterNetEvent("kaves_drugs:washMoney")
AddEventHandler("kaves_drugs:washMoney", function(coord, heading)
    local playerPed = PlayerPedId()
    ...
    DeleteEntity(money)
    DeleteEntity(money2)
    TriggerServerEvent("kaves_drugs:addMoney", washcount/2)
    washcount = 0
    washing = false
end)
