	...
	if (item.response == 'openTarget') {
		...
		$.each(item.data, function (index, item) {
			...
			$("#target-"+index).hover((e)=> {
				$("#target-"+index).css("color",e.type === "mouseenter"?"#4b7ab8":"white")
			})
...
$(document).on('mousedown', (event) => {
	...
	if (split[0] === 'target') {
		...
		switch (event.which) {
			case 1:
				$.post('http://labrp_Eye/selectTarget', JSON.stringify(Number(split[1]) + 1));
			case 3:
				$.post('http://labrp_Eye/closeTarget');
			break;
		}
	}
});
