...
Citizen.CreateThread(function()
    ...
    while true do
	    ...
	    if GetDistanceBetweenCoords(839.30, -2308.47, 30.64, pedCoords) < 60.0 then
            createoxyPed()
            if GetDistanceBetweenCoords(839.30, -2308.47, 30.64, pedCoords) < 1.5 then
			    --DrawText3Ds(-1563.51, -441.16, 36.97, "[E] $1500 - Delivery Job")
                TriggerEvent('cd_drawtextui:ShowUI', 'show', '<strong>[E]</strong> $1500 - Delivery Job', 'red')
        ...
        if foundcar == true then
            ...
            if GetDistanceBetweenCoords(mycoords.x, mycoords.y, mycoords.z, pedCoords) < 1.5 then
			    ...
			    elseif IsControlJustReleased(0,38) and boxesped == 0 then
                    --ESX.ShowHelpNotification("No more packages!")
                    exports['mythic_notify']:SendAlert('inform', 'No more packages!', 3500, { ['background-color'] = '#4287f5', ['color'] = '#ffffff' })
                elseif IsControlJustReleased(0,38) and not boxinhand then
                    --ESX.ShowHelpNotification("Is this a joke? Are you a cop! Im out of here!!")
                    exports['mythic_notify']:SendAlert('inform', 'Is this a joke? Are you a cop! Im out of here!!', 3500, { ['background-color'] = '#4287f5', ['color'] = '#ffffff' })
                    SetEntityAsNoLongerNeeded(pedCar)
                    SetEntityAsNoLongerNeeded(carpedDrive)
            ...
            if GetDistanceBetweenCoords(carcoords.x, carcoords.y, carcoords.z, pedCoords) < 2.0 then
                 if not IsPedInVehicle(PlayerPedId(), oxyVehicle, true) then
			        ...
			        elseif IsControlJustReleased(0,38) and boxinhand then
                        --ESX.ShowHelpNotification("You already have a box in your hand!")
                        exports['mythic_notify']:SendAlert('inform', 'You already have a box in your hand!', 3500, { ['background-color'] = '#4287f5', ['color'] = '#ffffff' })
                    end
                 end    
		    end	
        end
        ...
        if boxesped == 0 then 
            ...
            --ESX.ShowHelpNotification("You dont have anymore packages left. Dump the car it might be hot!")
            exports['mythic_notify']:SendAlert('inform', 'You dont have anymore packages left. Dump the car it might be hot!', 3500, { ['background-color'] = '#4287f5', ['color'] = '#ffffff' })
            SetEntityAsNoLongerNeeded(pedCar)
            SetEntityAsNoLongerNeeded(carpedDrive)
        ...
        if (not DoesEntityExist(oxyVehicle) or GetVehicleEngineHealth(oxyVehicle) < 100.0) and started then
            ...
            started = false
            Oxyrun = false
		    --ESX.ShowHelpNotification("Yo you ~r~destroyed ~w~the car man! I will get someone else to deilver the packages!")
            exports['mythic_notify']:SendAlert('inform', 'Yo you destroyed the car man! I will get someone else to deilver the packages!', 3500, { ['background-color'] = '#4287f5', ['color'] = '#ffffff' })
        end
        ...
        if IsPedDeadOrDying(carpedDrive, 1) and started and pedisspawned then
            ...
            test = false
            Oxyrun = false
            --ESX.ShowHelpNotification("Something happened to one of your clients another one is on the way!")
            exports['mythic_notify']:SendAlert('inform', 'Something happened to one of your clients another one is on the way!', 3500, { ['background-color'] = '#4287f5', ['color'] = '#ffffff' })
        end
        ...
        if gotdest and GetDistanceBetweenCoords(currentDestination.x, currentDestination.y, currentDestination.z, pedCoords) < 50.0 and started == true and pedisspawned == false then
                ...
                exports['mythic_notify']:SendAlert('inform', 'You are close to the drop off wait for your clients!', 3500, { ['background-color'] = '#4287f5', ['color'] = '#ffffff' })
                pedisspawned = true
...
function spawnOxyCar()
    ...
    --ESX.ShowHelpNotification("Go to the marker on your GPS and wait for your clients!")
    exports['mythic_notify']:SendAlert('inform', 'Go to the marker on your GPS and wait for your clients!', 3500, { ['background-color'] = '#4287f5', ['color'] = '#ffffff' })
