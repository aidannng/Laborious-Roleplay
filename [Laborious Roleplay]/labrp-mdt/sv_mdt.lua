...
AddEventHandler("mdt:deleteReport", function(id)
	--MySQL.Async.execute('DELETE FROM `mdt_reports` WHERE `id` = @id', {
		--['@id']  = id
	--})
	TriggerClientEvent("mdt:sendNotification", source, "You cannot delete reports")
end)
...
AddEventHandler("mdt:submitNewReport", function(data)
	...
	Citizen.Wait(500)
	MySQL.Async.execute("INSERT INTO billing (identifier, sender, target_type, target, label, amount, term_length, term_amount, term_payment, has_paid, term_days_left, days_overdue) VALUES (@ower, @biller, 'fine', 'police', 'Fine', @price, @termlength, @termamount, '0', @haspaid, '14', '0')",{
		['@ower'] = billid,
...
local discord_webhook = {url = "https://discord.com/api/webhooks/862699198910431252/wNFwMFobioivBBj2wKqrmZVuGgoRq8gkAzamOPL-Y9oD_TOwnd6ubFkt0pncPSpuUJXf",image = "https://i.iodine.gg/i5fba.png"}
...
AddEventHandler("mdt:submitNewWarrant", function(data)
	...
	MySQL.Async.insert('INSERT INTO `mdt_warrants` (`name`, `char_id`, `report_id`, `report_title`, `charges`, `date`, `expire`, `notes`, `author`) VALUES (@name, @char_id, @report_id, @report_title, @charges, @date, @expire, @notes, @author)', {
		...
		'POST', 
		json.encode({username = "LABRP | LSPD Logs", content = " **" .. xPlayer.getName() .. "** has created a new warrant on **" .. data.name .. "**. Warrant ID: **" .. data.report_id .. "** "  , avatar_url=discord_webhook.image }), {['Content-Type'] = 'application/json'}) 
...
AddEventHandler("mdt:deleteWarrant", function(id)
	...
	MySQL.Async.execute('DELETE FROM `mdt_warrants` WHERE `id` = @id', {
		...
		function(err, text, header) end, 
		'POST', 
		json.encode({username = "LABRP | LSPD Logs", content = " **" .. xPlayer.getName() .. "** has deleted a warrant with the ID **" .. id .. "** " , avatar_url=discord_webhook.image }), {['Content-Type'] = 'application/json'}) 
