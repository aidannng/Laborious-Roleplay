...
AddEventHandler('RS7x:Payout', function()
    ...
    while Robbing == true do
        if math.random(1,100) <=75 then
            xPlayer.addInventoryItem("black_money", math.random(70500,85521))
            xPlayer.addInventoryItem("gold", 7)
            Robbing = false
        else
            xPlayer.addInventoryItem("black_money", math.random(70275,85382))
            xPlayer.addInventoryItem("greenusb", 1)
           Robbing = false
        end
    end
end)
