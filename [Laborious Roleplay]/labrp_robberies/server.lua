...
AddEventHandler('openfleecadoor', function(data)
    TriggerClientEvent("fleeca:openvaultdoor", -1, data)
end)

RegisterServerEvent('closefleecadoor')
AddEventHandler('closefleecadoor', function(data)
    TriggerClientEvent("fleeca:closevaultdoor", -1, data)
end)


RegisterNetEvent('fleeca:reward')
AddEventHandler('fleeca:reward', function()
    local xPlayer = ESX.GetPlayerFromId(source)
    local fleecapayout = math.random(35000, 50000)
...
ESX.RegisterServerCallback('labrp:store:haslockpick', function(source, callback)
    local xPlayer = ESX.GetPlayerFromId(source)
    local money = xPlayer.getInventoryItem('lockpick').count
...
RegisterServerEvent('labrp:store:payout')
AddEventHandler('labrp:store:payout', function()
    local xPlayers = ESX.GetExtendedPlayers()
    xPlayer.addInventoryItem('black_money', math.random(800, 2000))
end)