            ...
            if (selectedObjectData != null) {
                ...
                $.post('http://qb-houses/setupMyObjects', JSON.stringify({}), function(myObjects){
                    ...
                    $.each(myObjects, function(i, object){
                        var elem = '<div class="decorate-item" id="myobject-'+i+'" data-type="myObject"><span id="decorate-item-name"><b>Nesne: </b>'+object.hashname+'</span><span id="decorate-item-category"><strong>Fiyat: </strong><span id="item-price" style="color: green;">Sahip</span></span></div>';
...
$(document).on('click', '.footer-btn', function(){
    ...
    if (selectedCategory != "remove-owned-obj") {
        ...
        $.each(houseCategorys[selectedCategory].items, function(i, item){
            var elem = '<div class="decorate-item" id="object-'+i+'" data-type="newObject"><span id="decorate-item-name"><b>Nesne: </b>'+(item.label).charAt(0).toUpperCase() +''+(item.label).substr(1).toLowerCase()+'</span><span id="decorate-item-category"><strong>Fiyat: </strong><span id="item-price" style="color: green;">€'+item.price+'</span></span></div>';
            $('.decorate-items').append(elem);
