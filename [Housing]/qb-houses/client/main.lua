...
AddEventHandler('qb-houses:client:addGarage', function()
    if closesthouse ~= nil then 
        ...
        TriggerEvent('notification', ('There is no home nearby..'), 2)
    end
end)
    ...
    if(GetDistanceBetweenCoords(pos, Config.Houses[closesthouse].coords.enter.x, Config.Houses[closesthouse].coords.enter.y, Config.Houses[closesthouse].coords.enter.z, true) < 1.5)then
        if hasKey then
            if Config.Houses[closesthouse].locked then
               ...
               TriggerEvent('notification', ('House lock is on.'), 3)
            else
                TriggerServerEvent('qb-houses:server:lockHouse', true, closesthouse)
                TriggerEvent('notification', ('House locked!'), 2)
            end
        else
            TriggerEvent('notification', ('You dont have key of this property..'), 2)
        end
    else
        TriggerEvent('notification', ('There is no door..'), 2)
    end
end)
            ...
            if(GetDistanceBetweenCoords(pos, Config.Houses[closesthouse].coords.enter.x, Config.Houses[closesthouse].coords.enter.y, Config.Houses[closesthouse].coords.enter.z, false) < 30)then
                ...
                if hasKey then
                    ...
                    if not inside then
                        if closesthouse ~= nil then
                            if(GetDistanceBetweenCoords(pos, Config.Houses[closesthouse].coords.enter.x, Config.Houses[closesthouse].coords.enter.y, Config.Houses[closesthouse].coords.enter.z, true) < 1.5)then
                                DrawText3Ds(Config.Houses[closesthouse].coords.enter.x, Config.Houses[closesthouse].coords.enter.y, Config.Houses[closesthouse].coords.enter.z, '~b~/enter~w~ - Enter house')
                            end
                        end
                    end
                    ...
                    if CurrentDoorBell ~= 0 then
                        if(GetDistanceBetweenCoords(pos, Config.Houses[closesthouse].coords.enter.x + POIOffsets.exit.x, Config.Houses[closesthouse].coords.enter.y + POIOffsets.exit.y, Config.Houses[closesthouse].coords.enter.z - Config.MinZOffset + POIOffsets.exit.z, true) < 1.5)then
                            DrawText3Ds(Config.Houses[closesthouse].coords.enter.x + POIOffsets.exit.x, Config.Houses[closesthouse].coords.enter.y + POIOffsets.exit.y, Config.Houses[closesthouse].coords.enter.z - Config.MinZOffset + POIOffsets.exit.z + 0.35, '~g~G~w~ -Open door')
                            ...
                            if POIOffsets ~= nil then
                                if POIOffsets.exit ~= nil then
                                    if(GetDistanceBetweenCoords(pos, Config.Houses[CurrentHouse].coords.enter.x + POIOffsets.exit.x, Config.Houses[CurrentHouse].coords.enter.y + POIOffsets.exit.y, Config.Houses[CurrentHouse].coords.enter.z - Config.MinZOffset + POIOffsets.exit.z, true) < 1.5)then
                                        DrawText3Ds(Config.Houses[CurrentHouse].coords.enter.x + POIOffsets.exit.x, Config.Houses[CurrentHouse].coords.enter.y + POIOffsets.exit.y, Config.Houses[CurrentHouse].coords.enter.z - Config.MinZOffset + POIOffsets.exit.z, '~g~E~w~ -Exit')
                                        DrawText3Ds(Config.Houses[CurrentHouse].coords.enter.x + POIOffsets.exit.x, Config.Houses[CurrentHouse].coords.enter.y + POIOffsets.exit.y, Config.Houses[CurrentHouse].coords.enter.z - Config.MinZOffset + POIOffsets.exit.z - 0.1, '~g~H~w~ - Look through the hole')
                        ...
                        if closesthouse ~= nil then
                            if(GetDistanceBetweenCoords(pos, Config.Houses[closesthouse].coords.enter.x, Config.Houses[closesthouse].coords.enter.y, Config.Houses[closesthouse].coords.enter.z, true) < 1.5)then
                                if not viewCam and Config.Houses[closesthouse].locked then
                                    DrawText3Ds(Config.Houses[closesthouse].coords.enter.x, Config.Houses[closesthouse].coords.enter.y, Config.Houses[closesthouse].coords.enter.z, '~g~E~w~ - Click to see the house')
                                    if IsControlJustPressed(0, Keys["E"]) then
                                        TriggerServerEvent('qb-houses:server:viewHouse', closesthouse)
                                    end
                    ...
                    if CurrentHouse ~= nil then
                        if stashLocation ~= nil then
                            if(GetDistanceBetweenCoords(pos, stashLocation.x, stashLocation.y, stashLocation.z, true) < 1.5)then
                                ...
                                if IsControlJustPressed(0, Keys["E"]) then
                                    TriggerEvent('m3:inventoryhud:client:openStash', 'Ev Deposu', 'all')
                    ...
                    if CurrentHouse ~= nil then
                        if outfitLocation ~= nil then
                            if(GetDistanceBetweenCoords(pos, outfitLocation.x, outfitLocation.y, outfitLocation.z, true) < 1.5)then
                                DrawText3Ds(outfitLocation.x, outfitLocation.y, outfitLocation.z, '~g~E~w~ - Clothes')
                                if IsControlJustPressed(0, Keys["E"]) then
                                    TriggerEvent('qb-clothing:client:openOutfitMenu')
                                end
                            elseif(GetDistanceBetweenCoords(pos, outfitLocation.x, outfitLocation.y, outfitLocation.z, true) < 3)then
                                DrawText3Ds(outfitLocation.x, outfitLocation.y, outfitLocation.z, 'Clothes')
                            end
                        end
                    end
                end
                    ...
                    if CurrentHouse ~= nil then
                        if logoutLocation ~= nil then
                            if(GetDistanceBetweenCoords(pos, logoutLocation.x, logoutLocation.y, logoutLocation.z, true) < 1.5)then
                                DrawText3Ds(logoutLocation.x, logoutLocation.y, logoutLocation.z, '~g~E~w~ - Exit')
                                if IsControlJustPressed(0, Keys["E"]) then
                                    DoScreenFadeOut(250)
                            ...
                            elseif(GetDistanceBetweenCoords(pos, logoutLocation.x, logoutLocation.y, logoutLocation.z, true) < 3)then
                                DrawText3Ds(logoutLocation.x, logoutLocation.y, logoutLocation.z, 'Exit')
                            end
                        end
                    end
                end
...
AddEventHandler('qb-houses:client:RingDoor', function(player, house)
    if closesthouse == house and inside then
        ...
        TriggerEvent('notification', ('Someone ringing the door.'), 2)
    end
end)
...
AddEventHandler('qb-houses:client:giveHouseKey', function(data)
    local xPlayer, distance = GetClosestPlayer()
    if xPlayer ~= -1 and distance < 2.5 and closesthouse ~= nil then
        ...
        if housedist < 10 then
            ...
            TriggerEvent('notification', ('You are not close enought to house..'), 2)
        end
    elseif closesthouse == nil then
        TriggerEvent('notification', ('No home nearby'), 2)
    else
        TriggerEvent('notification', ('Nobody nearby'), 2)
    end
end)
        ...
        if housedist < 5 then
            ESX.TriggerServerCallback('qb-houses:server:getHouseOwner', function(result)
                if ESX.GetPlayerData().identifier == result then
                   ...
                   TriggerEvent('notification', ('You are not owner of this house'), 2)
                end
            end, closesthouse)
        else
            TriggerEvent('notification', ('You are not close to house..'), 2)
        end
    else
        TriggerEvent('notification', ('You are not close to house..'), 2)
    end
...
function optionMenu(citizenData)
    ped = PlayerPedId();
    MenuTitle = "What's wrong?"
    ClearMenu()
    Menu.addButton("Delete house key", "removeHouseKey", citizenData) 
...
AddEventHandler('qb-houses:client:setLocation', function(data)
    ...
    if inside then
        if hasKey then
            ...
            TriggerEvent('notification', ('You are not in house.'), 2)
        end
    else    
        TriggerEvent('notification', ('You are not in house.'), 2)
    end
                        ...
                        if not Config.Houses[closesthouse].IsRaming then
                            ...
                            Skillbar.Start({
                                ...
                                if RamsDone + 1 >= Config.RamsNeeded then
                                    TriggerServerEvent('qb-houses:server:lockHouse', false, closesthouse)
                                    TriggerEvent('notification', ('It worked! Now, the door is open..'), 2)
                                ...
                                TriggerServerEvent('qb-houses:server:SetRamState', false, closesthouse)
                                 TriggerEvent('notification', ('No success. Pleast try again.'), 2)
                                DoRamAnimation(false)
                            end)
                            TriggerServerEvent('qb-houses:server:SetRamState', true, closesthouse)
                        else
                            TriggerEvent('notification', ('Someone is already working on door..'), 2)
                        end
                    else
                        TriggerEvent('notification', ('The house is already open..'), 2)
                    end
                else
                     TriggerEvent('notification', ('You are not close to any house..'), 2)
                end
            else
                TriggerEvent('notification', ('There is not enough police in city..'), 2)
            end
        end)
    else
        TriggerEvent('notification', ('You are not near any house..'), 2)
    end
end)
...
AddEventHandler('qb-houses:client:ResetHouse', function()
    ...
    if closesthouse ~= nil then
        ...
        if Config.Houses[closesthouse].IsRammed then
            ...
            TriggerServerEvent('qb-houses:server:lockHouse', true, closesthouse)
            RamsDone = 0
            TriggerEvent('notification', ('You locked house..'), 2)
        else
            TriggerEvent('notification', ('This door is already broken and open..'), 2)
        end
    end
end)