...
AddEventHandler("ped:intrunk", function(trunksent)
  intrunk = trunksent
  if intrunk then
    print("put inside trunk")
  else
    print("ejected from trunk")
  end
end)
...
AddEventHandler('client:pass', function(passType,args)
    ...
    if passInfo == nil then
      print("Failed to use " .. passType .. " with no permissions")
    ...
    print("Accepted " .. passType .. " with rank " .. rank)
    if rank < 1 then
