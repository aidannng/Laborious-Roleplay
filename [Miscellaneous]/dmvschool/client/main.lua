		...
		if data.current.value == 'theory_test' then
			...
			ESX.TriggerServerCallback('esx_dmvschool:canYouPay', function(haveMoney)
				if haveMoney then
					...
					exports['mythic_notify']:SendAlert('error', 'You do not have enough cash to start the test')
				end
			end, 'dmv')
		elseif data.current.value == 'drive_test' then
			...
			ESX.TriggerServerCallback('esx_dmvschool:canYouPay', function(haveMoney)
				if haveMoney then
					...
					exports['mythic_notify']:SendAlert('inform', 'You will be given the keys to the vehicle in 1 second!') 
					Citizen.Wait(1500)
	...
	while true do
		...
		if CurrentTest == 'drive' then
			...
			if Config.CheckPoints[nextCheckPoint] == nil then
				...
				exports['mythic_notify']:SendAlert('inform', 'You have finished the driving test')
		...
		if CurrentTest == 'drive' then
			...
			if IsPedInAnyVehicle(playerPed, false) then
				...
				for k,v in pairs(Config.SpeedLimits) do
					if CurrentZoneType == k and speed > v then
						...
						if not IsAboveSpeedLimit then
							...
							exports['mythic_notify']:SendAlert('error', 'You are driving too fast')
							exports['mythic_notify']:SendAlert('error', 'You have comitted ' .. DriveErrors .. '/' .. Config.MaxErrors .. ' mistakes')
						end
					end
				end
				...
				if health < LastVehicleHealth then
					...
					exports['mythic_notify']:SendAlert('error', 'You have damaged the vehicle. Be careful')
					exports['mythic_notify']:SendAlert('error', 'You have comitted ' .. DriveErrors .. '/' .. Config.MaxErrors .. ' mistakes')
