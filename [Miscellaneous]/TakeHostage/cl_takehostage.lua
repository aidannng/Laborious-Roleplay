...
function callTakeHostage()
	...
	if not canTakeHostage then 
		exports['mythic_notify']:SendAlert('error', 'You need a pistol with ammo to take a hostage at gunpoint!') 
	end
	...
	if not takeHostage.InProgress and canTakeHostage then			
		...
		if closestPlayer then
			...
			if targetSrc ~= -1 then
				...
				exports['mythic_notify']:SendAlert("error","No one nearby to take as hostage!")
			end
		else
			exports['mythic_notify']:SendAlert("error","No one nearby to take as hostage!")
		end
