...
ESX.RegisterServerCallback('bb-garages:server:getOwnedVehicles', function(source, cb, nearbyVehicles, freeSlots, name)
    ...
    MySQL.Async.fetchAll("SELECT * FROM `bbvehicles` WHERE `identifier` = '" .. xPlayer.getIdentifier() .. "'", {}, function(vehicles)
...
AddEventHandler('bb-garages:server:setFirstData', function()
    MySQL.Async.fetchAll("SELECT * FROM `bbvehicles`", {}, function(vehicles)
        if vehicles ~= nil and vehicles[tonumber("1")] ~= nil then
            local counter, impound = 0, 0
            for _, vehicle in pairs(vehicles) do
                if vehicle.state ~= 'unknown' and vehicle.state ~= 'impound' and vehicle.state ~= 'PDM' then
                ...
                elseif vehicle.state == 'unknown' then
                    MySQL.Async.execute("UPDATE `bbvehicles` SET `state` = 'impound', `parking` = '' WHERE `plate` = '" .. vehicle.plate .. "' AND `identifier` = '" .. vehicle.identifier .. "' LIMIT 1")
                end
            

                if vehicle.fakeplate ~= nil and vehicle.fakeplate ~= '' then
                    MySQL.Async.execute("UPDATE `bbvehicles` SET `fakeplate` = '' WHERE `plate` = '" .. vehicle.plate .. "' AND `identifier` = '" .. vehicle.identifier .. "' LIMIT 1")
                end
            end
...
AddEventHandler('bb-garages:server:impoundVehicle', function(plate)
    ...
    MySQL.Async.execute("UPDATE `bbvehicles` SET `state` = 'impound', `parking` = '' WHERE `plate` = '" .. plate .. "' AND `identifier` = '" .. xPlayer.getIdentifier() .. "' LIMIT 1")
end)
...
AddEventHandler('bb-garages:server:parkVehicle', function(garage, slots, plate, stats)
    ...
    MySQL.Async.fetchAll("SELECT `props` FROM `bbvehicles` WHERE `identifier` = '" .. xPlayer.getIdentifier() .. "' AND `plate` = '" .. plate .. "' LIMIT 1", {}, function(props)
...
AddEventHandler('bb-garages:server:vehiclePayout', function(garage, plate, price, typ)
    ...
    if xPlayer.getMoney() >= price then
        MySQL.Async.fetchAll("SELECT * FROM `bbvehicles` WHERE `identifier` = '" .. xPlayer.getIdentifier() .. "' AND `plate` = '" .. plate .. "' LIMIT 1", {}, function(vehicle)
            if vehicle[tonumber("1")] ~= nil then
                local veh = vehicle[tonumber("1")]

                xPlayer.removeMoney(price)
                MySQL.Async.execute("UPDATE `bbvehicles` SET `state` = 'unknown', `parking` = '' WHERE `id` = '" .. veh.id .. "'")
                if typ == 'garages' then
...
ESX.RegisterServerCallback('bb-garages:server:getImpoundedVehicles', function(source, cb, name)
    ...
    MySQL.Async.fetchAll("SELECT * FROM `bbvehicles` WHERE `identifier` = '" .. xPlayer.getIdentifier() .. "' AND `state` = 'impound'", {}, function(vehicles)
        function payButton(plate)
...
ESX.RegisterServerCallback('bb-garages:server:isVehicleOwned', function(source, cb, plate)
    local src = source
    local xPlayer = ESX.GetPlayerFromId(src)
    MySQL.Async.fetchAll("SELECT * FROM `bbvehicles` WHERE `identifier` = '" .. xPlayer.getIdentifier() .. "' AND `plate` = '" .. plate .. "' LIMIT 1", {}, function(result)
...
AddEventHandler('bb-garages:server:isPlayerVehicle', function(typ, plate, vehicle)
    if typ == 'STEAL' then
        MySQL.Async.fetchAll("SELECT `model` FROM `bbvehicles` WHERE `plate` = '" .. plate .. "' LIMIT 1", {}, function(result)
            if result[tonumber("1")] ~= nil then
                MySQL.Async.execute("UPDATE `bbvehicles` SET `fakeplate` = '%' WHERE `plate` = '" .. plate .. "' AND `model` = '" .. result[tonumber("1")].model .. "' LIMIT 1")
    ...
    elseif typ == 'SET' then
        if networkVehicles[vehicle] ~= nil then
            if networkVehicles[vehicle][tonumber("1")] == plate then
                MySQL.Async.execute("UPDATE `bbvehicles` SET `fakeplate` = '' WHERE `plate` = '" .. plate .. "' LIMIT 1")
                networkVehicles[vehicle] = nil
            else
                MySQL.Async.execute("UPDATE `bbvehicles` SET `fakeplate` = '" .. plate .. "' WHERE `plate` = '" .. networkVehicles[vehicle][tonumber("1")] .. "' LIMIT 1")
...
AddEventHandler('bb-garages:server:fakeplate:breakScrewdriver', function()
    ...
    TriggerClientEvent(Config['settings']['notification'], src, "Your Advanced Screwdriver went broken", 2)
end)
...
AddEventHandler('bb-garages:server:fakeplate:removeLicensePlate', function(slot)
    ...
    TriggerClientEvent(Config['settings']['notification'], src, "Success fully installed license plate", 2)
end)
...
AddEventHandler('bb-garages:server:fakeplate:createLicensePlate', function(plate)
    ...
    TriggerClientEvent(Config['settings']['notification'], src, "You stole a license plate", 2)
end)
    ...
    if Config['settings']['dev']['esx'] == '1.2' then
        ...
        AddEventHandler('bb-garages:server:dev:creategarage', function(coords, heading, args)
            ...
            if serverConfig['garages'][name] == nil then
                ...
                TriggerClientEvent(Config['settings']['notification'], src, "Created new garage! [" .. name .. "]", "success")
                TriggerClientEvent('bb-garages:client:syncConfig', -1, true, serverConfig)
            else
                TriggerClientEvent(Config['settings']['notification'], src, "A Garage with the same name already exists", "error")
            end
        end)
        ...
        AddEventHandler('bb-garages:server:dev:setinteract', function(coords, heading, args)
            ...
            if closestGarage ~= '' then
                ...
                if garages[closestGarage] ~= nil then
                    ...
                    TriggerClientEvent(Config['settings']['notification'], src, "Updated " .. closestGarage .. " interact location.", "success")
                    TriggerClientEvent('bb-garages:client:syncConfig', -1, true, serverConfig)
                else
                    TriggerClientEvent(Config['settings']['notification'], src, "Error: Could not find the garage " .. closestGarage .. " on the Database", "error")
                end
            else
                TriggerClientEvent(Config['settings']['notification'], src, "Error: Could not find closest garage.", "error")
            end
        end)
        ...
        AddEventHandler('bb-garages:server:dev:setpayment', function(coords, heading, args)
            ...
            if closestGarage ~= '' then
                ...
                if garages[closestGarage] ~= nil then
                    ...
                    TriggerClientEvent(Config['settings']['notification'], src, "Updated " .. closestGarage .. " payment information.", "success")
                    TriggerClientEvent('bb-garages:client:syncConfig', -1, true, serverConfig)
                else
                    TriggerClientEvent(Config['settings']['notification'], src, "Error: Could not find the garage " .. closestGarage .. " on the Database", "error")
                end
