...
function openVehicleMenu() {
  ...
  if (repair) {
    ...
    '<div id="repairText">REPAIR MODE</div><div id="repairButton" class="ripple" onclick="cancelRepair()">CANCEL</div> </div>';
  }
...
window.oncontextmenu = function (event) {
  ...
  if (element != null) {
    ...
    if (partType == "uninstalled") {
      ...
      html = html + '    <a onclick="openInstallPart()">Install</a>';
    } else {


      if (Parts[partType].health > 0 && allparts[partType][Parts[partType].type].repair != null) {
        html = html + '    <a onclick="repairPart()">Repair</a>';
         html = html + '    <a onclick="openRepairParts()">Inspect</a>';
      }

      html = html + '    <a onclick="openReplaceParts()">Replace</a>';
