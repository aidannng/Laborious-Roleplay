...
Citizen.CreateThread(function()
	...
	while true do
		...
		if (Config.allVehicles and veh ~= 0) or IsThisModelACar(GetEntityModel(veh)) then
...
AddEventHandler("lc_hud:triggerhud",function(status)
	if status == nil then
		showHud = not showHud
	else
		showHud = status
...
AddEventHandler('advanced_vehicles:repairCar', function(repair)
	for k,v in pairs(Config.repair[repair].repair) do
		...
		elseif v == "body" then
			...
			SetVehicleFixed(vehicleDataAction.veh)
			SetVehicleDirtLevel(vehicleDataAction.veh, 0)
		else
...
AddEventHandler('advanced_vehicles:showStatusUI', function()
	...
	if vehicleData ~= nil and vehicleData ~={} and vehicleData.loaded == true then
		...
		exports["mythic_notify"]:SendAlert("error", Lang[Config.lang]['invalid_veh'])

		--TriggerEvent("advanced_vehicles:Notify","negado",Lang[Config.lang]['invalid_veh'])
	end
end)
...
RegisterNUICallback('makeAction', function(data, cb)
	...
	if ismechanic then
		...
		exports["mythic_notify"]:SendAlert("error", Lang[Config.lang]['no_permission'])

		--TriggerEvent("advanced_vehicles:Notify","negado",Lang[Config.lang]['no_permission'])
	end
end)
...
RegisterNUICallback('notifyOil', function(data, cb)
	exports["mythic_notify"]:SendAlert("error", Lang[Config.lang]['services_notify'])
...
AddEventHandler('advanced_vehicles:useTheJackFunction', function(data,firstStep)
	if firstStep then
		...
		while veh ~= 0 do
			...
			DrawText3Ds(vehCoords.x, vehCoords.y, vehCoords.z + 0.6, Lang[Config.lang]['off_vehicle'])
		...
		while veh == 0 and distance < 3.0 do
			...
			DrawText3Ds(vehCoords.x, vehCoords.y, vehCoords.z + 0.6, Lang[Config.lang]['start_job'])
...
AddEventHandler('advanced_vehicles:startNitroRecharge', function()
	...
	exports["mythic_notify"]:SendAlert("error", Lang[Config.lang]['nitro_recharging'])

	--TriggerEvent("advanced_vehicles:Notify","importante",Lang[Config.lang]['nitro_recharging'])
	...
	if vehicleData and vehicleData.loaded and vehicleData.veh == tempveh and vehicleData.nitroAmount == 0 and vehicleData.nitroRecharges > 0 then
		...
		exports["mythic_notify"]:SendAlert("error", Lang[Config.lang]['nitro_ready'])

		--TriggerEvent("advanced_vehicles:Notify","importante",Lang[Config.lang]['nitro_ready'])
	end
end)
		...
		if isDriving then
			...
			if vehicleData.nitroAmount == 0 then
				if vehicleData.nitroRecharges > 0 then
					...
					exports["mythic_notify"]:SendAlert("error", Lang[Config.lang]['nitro_empty'])

					--TriggerEvent("advanced_vehicles:Notify","negado",Lang[Config.lang]['nitro_empty'])
				end
			end
