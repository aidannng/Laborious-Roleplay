...
window.addEventListener('message', function (event) {
    ...
    if (item.showmenu) {
        ...
        for (let k of item.vehicleData.servicesNUI) {
            history[c] = {name: k.name, img: k.img, hour: timeConverter(k.timer,item.format.location)+' ('+new Intl.NumberFormat(item.format.location, { maximumFractionDigits: 0, minimumFractionDigits: 0 }).format(k.km / 1.9)+' Miles)', action: Lang[lang]['service']}
            c++
        }
        ...
        $('#vehicle-km').empty();
        var miles = (item.vehicleData.km/1000)/1.9
        $('#vehicle-km').append(item.vehicleData.plate + ' - ' + new Intl.NumberFormat(item.format.location, { maximumFractionDigits: 0, minimumFractionDigits: 0 }).format(miles) + ' Miles');
		...
		$('.painel').append(zero + new Intl.NumberFormat(item.format.location, { maximumFractionDigits: 2, minimumFractionDigits: 2 }).format(item.km / 1.9) + ' Miles');
        ...
        if (item.oilHealth <= 5) {
            if (notifyOil == false) {
                notifyOil = true
                $.post("http://advanced_vehicles/notifyOil");
            }
...
function makeAction(event) {
    ...
    $.post("http://advanced_vehicles/makeAction", JSON.stringify({action:action,idname:idname,name:name,img:img}));
}

function repairCar(repair) {
    $.post("http://advanced_vehicles/repairCar", JSON.stringify({repair:repair}));
}

// FUNÇÃO PARA FECHAR A NUI 

function closeNui() {
    $.post("http://advanced_vehicles/close");
}
