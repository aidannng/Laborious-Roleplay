        ...
        if IsPedInAnyVehicle(ped, true) == false and lastTruck ~= 0 then
            ...
            if GetDistanceBetweenCoords(pos,markerCoords) < 5 then
                ...
                if GetDistanceBetweenCoords(markerCoords,pos) < 2 then
                    if IsControlJustPressed(0,190) or IsControlJustPressed(0,189) then
                    ...
                    if IsControlJustReleased(0,190) or IsControlJustReleased(0,189) then
                        TriggerServerEvent('saveArmPosition',lastTruck,start)
                    end
                    if IsControlPressed(0, 189) and start ~= 0.0 then
                        bedslide2('retract')
                    end
                end
            end         
        elseif has_value(vehs, GetEntityModel(veh)) and lastTruck ~= 0 then
            if IsControlJustPressed(0,190) or IsControlJustPressed(0,189) or IsControlJustPressed(0,21) or IsControlJustPressed(0,36) then
                if trucks[lastTruck] ~= nil and trucks[lastTruck]['bedslide'] ~= nil then
                    start = trucks[lastTruck]['bedslide']
                end
            end
            if IsControlPressed(0, 190) or IsControlPressed(0,21) then 
                bedslide('extend')
            end
            if IsControlJustReleased(0,190) or IsControlJustReleased(0,189) then
                TriggerServerEvent('saveArmPosition',lastTruck,start)
            end
            if (IsControlPressed(0, 189) or IsControlPressed(0, 36))  and start ~= 0.0 then
                bedslide('retract')
            end
        end
        Wait(0)
