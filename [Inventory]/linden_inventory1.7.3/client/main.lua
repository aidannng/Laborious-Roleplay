...
OpenShop = function(id)
	if not currentInventory and CanOpenInventory() and not CanOpenTarget(ESX.PlayerData.ped) then
		...
		exports['mythic_notify']:SendAlert('inform', 'Remember to left-click to purchase items! NO DRAGGING') 
	end
end
...
RegisterCommand('oSteal', function() 
	OpenTargetInventory() 
end)

RegisterNetEvent('StealPlayer')
AddEventHandler('StealPlayer', function()
    ExecuteCommand('oSteal')
end)
