		...
		if owner then
			...
			if type == 'trunk' or type == 'glovebox' then
				if Datastore[id] then
					...
					local result = exports.ghmattimysql:scalarSync('SELECT `identifier` from `bbvehicles` WHERE `plate` = @plate', {
						['@plate'] = plate
					})
					if result then
						if inventory then
		...
		if owner then
			...
			if inv == 'trunk' or inv == 'glovebox' then
				...
				if data then
					...
					local plate = string.match(id, "-(.*)")
					if Config.TrimPlate then plate = func.trim(plate) end
					local owned = exports.ghmattimysql:scalarSync('SELECT `plate` FROM `bbvehicles` WHERE plate = @plate', {
						['@plate'] = plate
					})
					if owned == nil then
