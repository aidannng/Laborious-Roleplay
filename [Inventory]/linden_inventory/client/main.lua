...
RobTargetInventory = function(target)
	if not invOpen and not IsPedInAnyVehicle(ESX.PlayerData.ped, true) and not currentInventory and CanOpenInventory() then
		local targetPlayer, targetDistance
		if target then
			targetPlayer = target
...
exports('RobTargetInventory', OpenTargetInventory)
								...
								if IsControlJustPressed(1,38) then
									ESX.TriggerServerCallback('esx_license:checkLicense', function(hasWeaponLicense)
										if hasWeaponLicense then
											...
											TriggerEvent('mythic_notify:client:SendAlert', {type = 'error', text = 'You must buy a weapons license from a police officer! Head down to MRPD now and speak to the police!', length = 5000})
										end
...
RegisterCommand('steal', function() RobTargetInventory() end)
