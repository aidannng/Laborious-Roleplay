...
Citizen.CreateThread(function()
    while true do
        Citizen.Wait(0)
        for i,self in pairs(nearTextsV2) do
            if self.rendering and not self.destroyed and self.stopRendering == false then
                if self.isIn then
...
function create3DText(text, resName)
    ...
    self.destroy = function()
        ...
        self.destroyed = true
        self.update(true)
        dprint('Deleted text V2 %s', self.getId())
    end
    ...
    self.on = function(type, cb)
        ...
        elseif string.lower(type) == 'key' then
            ...
            dprint('Cannot create on state at 3D text because invalid state %s', self.state)
        end
        self.update()
    end
        ...
        if destroy then
            for ind,v in pairs(nearTextsV2) do
                if v.getId() == self.getId() then
                    nearTextsV2[ind] = nil
                    dprint('Deleted %s text from near table', self.getId())
                end
            end
            ...
            for ind,v in pairs(textsV2) do
                if v.getId() == self.getId() then
                    textsV2[ind] = nil
                    dprint('Deleted %s text from master table', self.getId())
                end
            end
        else
            for ind,v in pairs(textsV2) do
    ...
    dprint('Create new 3D text V2 %s', self.getId())
