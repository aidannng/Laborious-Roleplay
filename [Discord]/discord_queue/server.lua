...
function CheckForGhostUsers()
  for license, data in pairs(playerConnecting) do 
    ...
    if Config.Debug then 
      print("[Laborious Roleplay] (MASS DEBUG)")
      print("[Laborious Roleplay] PlayerName: " .. tostring(name) .. " || ID:" .. tostring(user) .. " || GetPlayerName(user): " .. tostring(GetPlayerName(user) ) );
    end
    --[[ MASS DEBUG END ]]--
  ...
  while true do 
    ...
    for license, data in pairs(playerConnecting) do 
      ...
      if GetPlayerName(user) ~= name and GetPlayerName(user) == nil then 
        ...
        Queue:PopLicense(license)
        if Config.Debug then 
          print("[Laborious Roleplay] (Thread : NO LONGER EXISTS) Popped player " .. name .. " from queue...");
        end
        if connecting[license] ~= nil then 
          connecting[license] = nil;
          if (currentConnectors > 0) then 
        ...
        if Config.Debug then 
          print("[Laborious Roleplay] (Thread : NO LONGER EXISTS) currentConnectors is == " .. tostring(currentConnectors) )
        end
        playerConnecting[license] = nil;
      end
    end
      ...
      if (GetPlayerCount() + 1) > slots or (GetPlayerCount() + Queue:GetMax()) > slots then  
        ...
        if not Queue:IsSetUp(user) then 
          ...
          if GetPlayerName(user) ~= nil then 
            sendToDiscQueue("QUEUED USER", "Player `" .. GetPlayerName(user):gsub("`", "") .. "` has been added to the queue...", "Laborious Roleplay ");
          end
        ...
        if connecting[license] == nil or connecting[license] ~= true then 
          ...
          Wait(1000);
          if GetPlayerName(user) ~= nil then
            sendToDisc("NEW CONNECTOR", "Player `" .. GetPlayerName(user):gsub("`", "") .. "` is allowed to join now!", "Laborious Roleplay ");
          end
...
AddEventHandler('playerDropped', function (reason)
  ...
  if (Queue:IsSetUp(user)) then 
    Queue:Pop(user);
    sendToDiscQueue("REMOVED QUEUE USER", "Player `" .. GetPlayerName(user):gsub("`", "") .. "` has been removed from the queue...", "Laborious Roleplay ");
