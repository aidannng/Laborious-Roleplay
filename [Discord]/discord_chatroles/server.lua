...
function sendMsg(firstline, msg, to) 
	TriggerClientEvent('chat:addMessage', to, {
		template = '<div style="padding: 0.5vw; margin: 0.5vw; background-color: rgba(55, 69, 95, 0.5); border-radius: 3px;">{0} <br>^0OOC: {1}</div>',
        args = { firstline, msg }
	});
end 
...
AddEventHandler('chatMessage', function(source, name, msg)
	...
	if not string.find(args[1], "/") and setContains(roleTracker, GetPlayerIdentifiers(source)[1]) and 
		...
		if hasColors then
			...
			if not has_value(allowedColors, tonumber(roleNum)) then
				dontSend = false
				TriggerClientEvent('chatMessage', source, "^7[^4LABRP^7] ^1You cannot use colored chat since you are not a donator...")
			end
		end
		if hasRed then
			-- Check if they have required role
			if not has_value(allowedRed, tonumber(roleNum)) then
				dontSend = false
				TriggerClientEvent('chatMessage', source, "^7[^4LABRP^7] ^1You cannot use the color RED in chat since you are not staff...")
			end
		end
		if hasEmoji then 
			if not has_value(allowedEmoji, tonumber(roleNum)) then 
				dontSend = false;
				TriggerClientEvent('chatMessage', source, "^7[^4LABRP^7] You cannot use emojis :(")
			end
		end
	...
	if not string.find(args[1], "/") and not has_value(inStaffChat, GetPlayerIdentifiers(source)[1]) and 
		...
		if hasEmoji then 
			if not has_value(allowedEmoji, tonumber(roleNum)) then 
				dontSend = false;
				TriggerClientEvent('chatMessage', source, "^7[^4LABRP^7] ^1You cannot use emojis :(")
			end
		end
		local theirColor = chatcolorTracker[source];
		local finalMessage = msg;
