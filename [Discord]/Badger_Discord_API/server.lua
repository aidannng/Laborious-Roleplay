...
Citizen.CreateThread(function()
	...
	print("[^4Laborious Roleplay^0] Synced with Discord API successfully");
end)
...
AddEventHandler('Badger_Discord_API:PlayerLoaded', function()
	if (GetCurrentResourceName() ~= "Badger_Discord_API") then 
		--TriggerClientEvent('chatMessage', -1, '^1[^5SCRIPT ERROR^1] ^3The script ^1' .. GetCurrentResourceName() .. ' ^3will not work properly... You must '.. 'rename the resource to ^1Badger_Discord_API');
	end
	local license = ExtractIdentifiers(source).license;
	if (tracked[license] == nil) then 
		tracked[license] = true;
		--TriggerClientEvent('chatMessage', source, '[^4Laborious Roleplay^1] Synced with Discord API successfully')
	end
end)

card = '{"type":"AdaptiveCard","$schema":"http://adaptivecards.io/schemas/adaptive-card.json","version":"1.2","body":[{"type":"Image","url":"' .. Config.Splash.Header_IMG .. '","horizontalAlignment":"Center"},{"type":"Container","items":[{"type":"TextBlock","text":"Laborious Roleplay","wrap":true,"fontType":"Default","size":"ExtraLarge","weight":"Bolder","color":"Light","horizontalAlignment":"Center"},{"type":"TextBlock","text":"' .. Config.Splash.Heading1 .. '","wrap":true,"size":"Large","weight":"Bolder","color":"Light", "horizontalAlignment":"Center"},{"type":"TextBlock","text":"' .. Config.Splash.Heading2 .. '","wrap":true,"color":"Light","size":"Medium","horizontalAlignment":"Center"},{"type":"ColumnSet","height":"stretch","minHeight":"100px","bleed":true,"horizontalAlignment":"Center","columns":[{"type":"Column","width":"stretch","items":[{"type":"ActionSet","actions":[{"type":"Action.OpenUrl","title":"Store","url":"' .. Config.Splash.Store_Link .. '","style":"positive"}]}],"height":"stretch"},{"type":"Column","width":"stretch","items":[{"type":"ActionSet","actions":[{"type":"Action.OpenUrl","title":"Website","style":"positive","url":"' .. Config.Splash.Website_Link .. '"}]}]}]},{"type":"ActionSet","actions":[{"type":"Action.OpenUrl","title":"Click to join our Discord","style":"destructive","iconUrl":"","url":"https://discord.labrp.net"}]}],"style":"default","bleed":true,"height":"stretch","isVisible":true}]}'
...
function IsDiscordEmailVerified(user) 
    ...
    if discordId then 
        ...
        if member.code == 200 then
        	...
        	print("[^4Laborious Roleplay^0] ERROR: Code 200 was not reached. Error provided: " .. member.data)
        end
    end
    return isVerified;
end
...
function GetGuildIcon()
	...
	if guild.code == 200 then
		...
		print("[^4Laborious Roleplay^0] An error occured, please check your config and ensure everything is correct. Error: "..(guild.data or guild.code)) 
	end
	return nil;
end
...
function GetDiscordAvatar(user) 
	...
	if discordId then 
		...
		print("[^4Laborious Roleplay^0] ERROR: Discord ID was not found...")
	end
    return imgURL;
end
...
function GetDiscordRoles(user)
	...
	if discordId then
		...
		if member.code == 200 then
			...
			print("[^4Laborious Roleplay^0] ERROR: Code 200 was not reached... Returning false. [Member Data NOT FOUND]")
			return false
		end
	else
		print("[^4Laborious Roleplay^0] ERROR: Discord was not connected to user's Fivem account...")
		return false
	end
	return false
end
...
Citizen.CreateThread(function()
	...
	if guild.code == 200 then
		local data = json.decode(guild.data)
		print("[^4Laborious Roleplay^0] Permission system guild set to: "..data.name.." ("..data.id..")")
	else
		print("[^4Laborious Roleplay^0] An error occured, please check your config and ensure everything is correct. Error: "..(guild.data or guild.code)) 
	end
end)