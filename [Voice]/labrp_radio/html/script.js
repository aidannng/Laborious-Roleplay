$(function(){
    ...
    $(".toggleStatus").on('click', function(e){
        ...
        if (status == false) {
            ...
            if ($(".channel_on").val() > 0) {
                $.post('http://labrp_radio/joinChannel', JSON.stringify({channel:$(".channel_on").val()}))
            }
        } else {
            $(".radio_on").hide();
            $(".radio_off").show();
            ...
            $.post('http://labrp_radio/leaveChannel', JSON.stringify({}))
        }
    })
    ...
    $(".volumeUp").on('click', function(e){
        ...
        vol = cvolume*0.01
        if (vol == 1) vol = 0.99; 
        $.post('http://labrp_radio/setVolume', JSON.stringify({volume:vol}))
        changeActiveVolume(cvolume);
    })
    $(".volumeDown").on('click', function(e){
        ...
        if (click.paused) {click.play();} else {click.currentTime=0}
        cvolume = cvolume - 10
        $.post('http://labrp_radio/setVolume', JSON.stringify({volume:cvolume*0.01}))
        changeActiveVolume(cvolume);
    })
    ...
    $(document).on('keypress', 'input', function(e){
        if ($(".channel_on").is(":focus") && e.which == 13) {
            ...
            if (inputVal > 0) {
                $.post('http://labrp_radio/joinChannel', JSON.stringify({channel:inputVal}))
            }
        }
    })
    ...
    document.onkeyup = function(data){
        if (data.which == 27) {
            $.post('http://labrp_radio/close', JSON.stringify({}));
        }
    }
