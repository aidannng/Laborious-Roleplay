...
AddEventHandler('house:attempt', function()
 ...
 elseif not isNight() and not isRobbing then
  ...
  --TriggerEvent('notification', 'It\'s too bright out', 1)
  exports['mythic_notify']:SendAlert('error', 'Something doesn\'t seem right?!')
 else
...
AddEventHandler('houseRobberies:createHouse', function(id)
 ...
 while isRobbing do
  ...
  if GetDistanceBetweenCoords(playerCoords, house.x+3.6, house.y-15, house.z-50, true) < 2.5 then
    exports['cd_drawtextui']:ShowInteraction('show', 'IndianRed', "[E] Exit House")
   if IsControlJustPressed(0, 38) then
    TriggerEvent('houseRobberies:deleteHouse', id)
   end
  elseif GetDistanceBetweenCoords(playerCoords, house.x+3.6, house.y-15, house.z-50, true) >= 2.55 and GetDistanceBetweenCoords(playerCoords, house.x+3.6, house.y-15, house.z-50, true) < 2.6 then
