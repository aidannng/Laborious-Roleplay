...
RegisterServerEvent('buyid')
AddEventHandler('buyid', function()
    local xPlayer = ESX.GetPlayerFromId(source)
    local money = xPlayer.getInventoryItem('money').count
    xPlayer.addInventoryItem('identification', 1)
    TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'inform', text = 'Here is your new ID', length = 4500, style = { ['background-color'] = '#05b5f9', ['color'] = '#FFFFFF' } })
end)
