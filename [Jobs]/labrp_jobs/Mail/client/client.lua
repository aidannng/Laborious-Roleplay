    ...
    while true do
        ...
        if isDelivering then
            ...
            if not notified and insideDeliveryArea then
                ...
                exports['mythic_notify']:SendAlert('inform', 'You are close to the delivery point. You can get out of your vehicle and take out the package from the back and deliver it.') 
                notified = true
            end
        end
    end
...
Citizen.CreateThread(function()
    lockerZone:onPlayerInOut(function(isPointInside, point)
        ...
        if insideLocker and hasJob then
            exports['cd_drawtextui']:ShowInteraction('show', 'Tomato', strings.lockerAction)
        else
    ...
    vehSpawnZone:onPlayerInOut(function(isPointInside, point)
        ...
        if insideSpawn then
            ...
            if not inVeh and isOnDuty then
                exports['cd_drawtextui']:ShowInteraction('show', 'Tomato', strings.vehSpawnAction)
            else
                exports['cd_drawtextui']:ShowInteraction('show', 'Tomato', strings.vehSpawnReturn)
            end
        else
            exports['cd_drawtextui']:HideInteraction()
        end
...
AddEventHandler('luke_maildelivery:VehicleHandle', function(vehicle)
    if workVehicle == nil then
        ...
        exports['mythic_notify']:SendAlert('error', 'You already have work vehicle out.')
    end
end)
...
function StartDelivery(deliveries)
    ...
    exports['mythic_notify']:SendAlert('inform', 'Drive to the delivery point and deliver the package from the back.')
    ...
    deliveryZone:onPlayerInOut(function(isPointInside, point)
        ...
        if insideDelivery then
            exports['cd_drawtextui']:ShowInteraction('show', 'Tomato', strings.doorAction)
        else
            exports['cd_drawtextui']:HideInteraction()
        end
    end)
        ...
        if insideDeliveryArea then
            ...
            if not zoneCreated and not IsPedInVehicle(playerPed, workVehicle, false) then
                ...
                vehicleBack:onPlayerInOut(function(isPointInside, point)
                    ...
                    if insideTrunk then
                        exports['cd_drawtextui']:ShowInteraction('show', 'Tomato', strings.trunkAction)
                    else
                        exports['cd_drawtextui']:HideInteraction()
                    end
                end)
