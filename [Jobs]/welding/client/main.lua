...
function StopNPCJob(cancel)
    ...
    if cancel then
	  ESX.ShowNotification(_U('cancel_mission'))
		...
		TriggerServerEvent('esx_welding:GiveItem')
		StartNPCJob()
		Done = true
	end
end
           	...
           	if GetDistanceBetweenCoords(coords, zone.Pos.x, zone.Pos.y, zone.Pos.z, true) < 3 then
                ...
                if IsControlJustReleased(1, Keys["E"]) and PlayerData.job ~= nil then
					if IsPedInAnyVehicle(GetPlayerPed(-1), false) and GetPedInVehicleSeat(GetVehiclePedIsIn(GetPlayerPed(-1), true), -1) == GetPlayerPed(-1) then
						exports['mythic_notify']:SendAlert('error', 'You cannot weld in the vehicle.')
					else
...
function VehicleMenu()
    ...
    local elements = {
        {label = Config.Vehicles.car.Label, value = Config.Vehicles.car}
    }
...
AddEventHandler('esx_welding:hasEnteredMarker', function(zone)
...
AddEventHandler('esx_welding:hasExitedMarker', function(zone)

	if zone == 'Vente' then
		TriggerServerEvent('esx_welding:stopVente')
	end
    CurrentAction = nil
	ESX.UI.Menu.CloseAll()
end)
	...
	while true do
		...
		if PlayerData.job ~= nil then
			...
			if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
				...
				TriggerEvent('esx_welding:hasEnteredMarker', currentZone)
			end
			if not isInMarker and HasAlreadyEnteredMarker then
				HasAlreadyEnteredMarker = false
				TriggerEvent('esx_welding:hasExitedMarker', LastZone)
			end
		end
	end
end)
            ...
            if (IsControlJustReleased(1, Keys["E"]) or IsControlJustReleased(2, Keys["RIGHT"])) and PlayerData.job ~= nil then
				...
				if PlayerData.job.name == Config.nameJob then
					...
					if CurrentAction == 'vente' then
						TriggerServerEvent('esx_welding:startVente')
						TaskStartScenarioInPlace(playerPed, "WORLD_HUMAN_CLIPBOARD", 0, 1, true)
						Wait(6000)
						ClearPedTasksImmediately(playerPed)
					end
					if CurrentAction == 'delete_vehicle' then
					  ...
					  if string.find (plate, platePrefix) then
						local car = Config.Vehicles.car

						if hash == GetHashKey(car.Hash) then
	...
	while true do
		...
		if IsControlJustReleased(1, Keys["F10"]) and PlayerData.job ~= nil and PlayerData.job.name == Config.nameJob then
			...
			if Onjob then
				...
				if IsPedInAnyVehicle(playerPed,  false) and IsVehicleModel(GetVehiclePedIsIn(playerPed,  false), GetHashKey("riata")) then
					StartNPCJob()
					Onjob = true
				else
